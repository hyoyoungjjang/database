...
--박나라 사원과 같은 직급코드, 같은 사수를 가지고있는 사원들의 사번, 사원명,
-- 직급코드, 사수번호
...
--> 각 직금별 최소급여를 받는 사원을 조회 (사번, 사원명, 직급코드, 급여)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
--단, 보너스포함 연봉이 3000만원 이상인 사람들만 조회
SELECT ROWNUM, EMP_ID,EMP_NAME,(SALARY+(SALARY * NVL(BONUS,0))) * 12 AS"보너스포함연봉", DEPT_CODE
FROM EMPLOYEE
...
WHERE ROWNUM <= 5;
--각 부서별 평균급여가 높은 3개의 부서 부서코드, 평균급여 조회
...
/*
    *순위를 매기는 함수(WINDOW FUNCTION)
    ...
    DENSE_RANK() OVER(정렬기준) : 동일한 순위가 있다고해도 그 다음 등수를 무조건 1씩 증가시킴
    
    무조건 SELECT절에서만 사용가능
*/

--급여가 높은 수서대로 순위를 매겨서 조회
