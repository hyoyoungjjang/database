...
SELECT EMP_NAME,SALARY*12AS"연봉",(SALARY + (SALARY*NVL(BONUS, 0))) * 12 AS"총수령액"
...
WHERE SAL_LEVEL = 'S1';
--8. EMPLOYEE테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원의 이름,
--   월급, 실수령액, 고용일 조회
SELECT EMP_NAME, SALARY, ((SALARY + (SALARY*NVL(BONUS, 0))) * 12 - ((SALARY*12)*0.03))AS"실수령액",
        HIRE_DATE
FROM EMPLOYEE
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '____\_%'  ESCAPE '\' AND (DEPT_CODE = 'D9'
      OR DEPT_CODE = 'D6') AND HIRE_DATE >= '90/01/01' AND HIRE_DATE <= '00/12/01'
      AND SALARY >= 2700000;
