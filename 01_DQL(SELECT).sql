...
-- EMPLOYEE테이블의 사원명, 사원의 연뵹(SALARY * 12)을 조회
 ...
 FROM EMPLOYEE;
 -- 산술연산 과정중에 NULL데이터가 포함되어 있다면 무조건 결과값은 NULL이다.
...
/*
    <컬럼명에 별칭 지정하기>
    산술연산을 하게되면 컬럼명이 지저분해준다. 이때 컬럼명에 별칭을 부여해서 깔끔하게 가져올 수 있다.
    [표현법]
...
FROM EMPLOYEE;
 ...
 /*
     <리터럴>
     임의로 지정한 문자열 ('')
     조회된 겨로가(RESULT SET)의 모든 행에 반복적으로 출력
 */
...
FROM EMPLOYEE;
 ...
 /*
    <DISTINCT>
    중복제거 - 컬럼에 표시된 값들을 한번씩만 조회하고자할 때 사용
 */
...
--위처럼 작성하면 에러가 발생한다. DISTINCT는 한번만 사용이 가능
...
--월급이 400만원 이상인 사원들의 사원명, 부서코드, 급여
...
/*
    ...
    조건A OR 조건 B -> 조건ADHK조건B중 하나라도 만족하는 값을 가지고 온다.
    
    <BETWEEN AND>
    조건식에 사용되는 구문
    몇이상 몇이하인 범위에 대한 조건을 제시할 때 사용하는 연산자(이상, 이하만 가능)
    [표현법]
    비교대상 컬럼 BETWEEN 하얀값 AND 상한값
*/
...
SELECT *
FROM EMPLOYEE
--WHERE HIRE_DATE >= '90/01/01' AND HIRE_DATE <= '01/01/01'; --DATE 타입도 비교연산이 가능하다.
...
/*
    ...
    EX) 비교할대상컬럼 LIKE '문자%' : 비교대상컬럼값 중에서 해당문자로 시작하는 값들만 조회
        ...
        EX) 비교할대상컬럼 LIKE '_문자' : 비교대상컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
            비교할대상컬럼 LIKE '문자_' : 비교할대상컬럼값 문자뒤에 아무글자나 한글자가 있는 값들 조회
             비교할대상컬럼 LIKE '_문자_' : 비교할대상컬럼값 문자앞뒤에 아무글자나 한글자가 있는 값들 조회
*/

--사원들중에 성이 전씨이니 사원들의 사원명, 급여, 입사일 조회
SELECT EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%';

--사원들중에서 이름에 '하'라는 글자가 포함된 사원의 이름, 전화번호 목록 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

--사원들중에 이름에 중간글자가'하'인 사원의 이름 전화번호 목록을 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
...
--부서배치를 아직 받지 않았고 보너스를 받은 사원들의 이름, 보너스, 부서코드 조회
...
--3. 입사일이 '95/01/01'이상이고 부서배치를 받지않은 사원들의 사번, 사원명, 입사일, 부서코드조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, DEPT_CODE
FROM EMPLOYEE
WHERE HIRE_DATE >= '95/01/01' AND DEPT_CODE IS NULL;

--4. 급여가 200만원 이상이고 500만원 이하인 사원중에서 입사일이 '01/01/01'이상이고 보너스를 받지 않는 사원들의
