...
/*
    ...
    FLOOR(NUMBER)
*/
SELECT FLOOR(123.856) FROM DUAL;
...
SELECT EMP_NAME, EMP_NO, 
JOB_CODE, DEPT_CODE, SALARY, BONUS
FROM EMPLOYEE
...
/*
    *EXTRACT: 특정 날짜로부터 년도|월|일 값을 추출해서 반환하는 함수
    ...
    [표현법]
    EXTRACT(YEAR EROM DATE) : 연도만 추출
    EXTRACT(MONTH EROM DATE) : 월만 추출
    EXTRACT(DAY EROM DATE) : 일만 추출
        => 결과는 NUMBER
*/
...
/*
    [형변환 함수]
    *TO_CHAR : 숫자 타입 또는 날짜 타입의 값을 문자타입으로 반환시켜주는 함수
...
SELECT TO_CHAR(1234, '99999') AS"NUMBER"FROM DUAL; -- 9의 자리수만큼 공간확보, 오른쪽 정렬, 빈칸공백
SELECT TO_CHAR(1234, '00000') AS"NUMBER"FROM DUAL; -- 0의 자리수만큼 공간확보, 오른쪽정력, 빈칸을 0으로 채움
SELECT TO_CHAR(1234, 'L99999') FROM DUAL; -- 현제 설정된 나라의 로컬 화폐단위 포함
...
SELECT HIRE_DATE,TO_CHAR(HIRE_DATE, 'YYYY') FROM EMPLOYEE;
...
-- 사원들의 사원명과 부서배치여부(배정완료 또는 미배정표시) 조회
SELECT EMP_NAME,DEPT_CODE, NVL2(DEPT_CODE, '배정완료', '미배정')
FROM EMPLOYEE;

--*NULLIF(비교대상1, 비교대상2)
-- 두값이 일치하면 NULL 일치하지 않는다면 비교대상1 반환
...
/*
    [선택함수]
    *DECODE(비교하고자하는 대상(컬럼, 연산식, 함수식), 비교값 1,결과값1 ,비교값2,결과값2, 비교값3,결과값3...)
...
--J7 사원의 급여를 10%인상 (SALARY * 1.1)
--J6 사원의 급여를 15%인상 (SALARY * 1.5)
--J5 사원의 급여를 12%인상 (SALARY * 1.2)
--그외 사원들은 급여를 5%인상(SALARY * 1.05)

SELECT EMP_NAME, SALARY AS "인상전",DECODE(JOB_CODE, 'J7',SALARY * 1.1,'J6',SALARY * 1.5,
...
--2. AVG(NUMBER) :  해당 컬럼값들의 평균값 구해서 반환
...
--4. MAX(모든타입가능) :  해당컬럼값들 중 가장 큰 값 구해서 반환
