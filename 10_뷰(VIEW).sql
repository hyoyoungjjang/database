...
--일본에서 근무하는 사원들이 사번, 이름, 부서명, 급여, 근무국가명 조회
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME,BONUL
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
    JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
    JOIN NATIONAL USING(NATIONAL_CODE);
...
/*
    *뷰 컬럼에 별칭부여
    서브쿼리의 SELECT절에 함수식이나 선술연산식이 기술되어있을 경우 반드시 별칭을 지정해야한다.
*/
...
/*
    ...
    1) OR REPLACE : 기준에 동일한 뷰가 있을 경우 갱신하고, 존재하지 않을 경우 새로 생성해라
    2) FORCE | NOFORCE
...
--200번 사원의 급여를 200만원으로 변경(WHERE SALARY >= 3000000 조건에 맞지않는 변경)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
AS SELECT * 
    ...
    WHERE SALARY >= 3000000
WITH CHECK OPTION;
--200번 사원의 급여를 200만원으로 변경(WHERE SALARY >= 3000000 조건에 맞지않는 변경)이 되지않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
