/*
    ...
    (무작정 다 조회해 오는게 아니라 테이블간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회해아한다.)
...
--일차하는 값이 없는 행은 조회에서 제외된 것을 확인할 수 있다.
...
--2. DEPARTMENT와 LOCATION 테이블을 참고하여 전체 부서의 부서코드
--   , 부서명, 지역코드, 지역명 조회
--> 오라클
...
-- ANSI 
 ...
 --3) FULL JOIN : 두 테이블이 가진 모든 행을 조회할 수 있다 (오라클 X)
 -- ANSI
  SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
 FROM EMPLOYEE
...
FROM EMPLOYEE E, DEPARTMENT D, JOB J, NATIONAL N, SAL_GRADE S, LOCATION L
WHERE DEPT_CODE = DEPT_ID
AND E.JOB_CODE = J.JOB_CODE
