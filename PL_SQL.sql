/*
    ...
    *PL/SQL 구조
    ...
    -[예외처리부] : EXCEPTION으로 시작, 예외 발생시 해결하기위한 구문
*/
...
BEGIN
    --사용자가 입력한 사번의 사원의 사번, 사원명, 직급코드, 급여,
    --부서명 조회 후 각 변수에 담아서 출력
...
--2. BEGIN 실행부
--<조건문>
-- 1) IF 조건문 THEN 실행내용 END IF; (IF문만 단독으로 사용할 때)
...
----------------실습------------------------
--사용자에게 입력받은 사번의 사원의 급여를 조회해서 SAL 변수에대입
-- 500만원 이상이면 '고급'
-- 400만원 이상이면 '중급'
-- 300만원 이상이면 '초급'
-- '해당 사원의 급여등급은  XX입니다.'
...
BEGIN
    ...
    IF SAL >= 5000000 THEN D := '고급';
    ELSIF SAL >= 4000000 THEN D := '중급';
    ELSIF SAL >= 3000000 THEN D := '초급';
    ELSE D := '인턴';
    END IF;
    
    DBMS_OUTPUT.PUT_LINE('해당 사원의 급여등급은 ' || D || '입니다');
...
--UNIQUE 제약조건 위배
--ALTER TABLE EMPLOYY ADD PRIMARY KEY(EMP_ID);
