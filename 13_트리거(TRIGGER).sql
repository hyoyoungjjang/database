/*
    ...
    *트리거 생성 구문
    ...
    [표현식]
    CREATE [OR REPLACE] TRIGGER 티리거명
...
--어떤 상품이 어떤 날짜에 몇개가 입고 또는 출고가 되는지
...
--이력번호 매번 새로운 번호를 발생시켜 돌아갈 수 있게 도와줄 시퀀스 생성
...
INSERT INTO TB_PRODETAIL 
VALUES(SEQ_DCODE.NEXTVAL,205,SYSDATE,5,'입고');
...
--TB_PRODUCT테이블에 매번 자동으로 재고수량 UPDATE 트리거 작성

/*
    -상품이 입고된 경우 -> 해당 상품을 찾아서 재고수량 증가UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK + 현재 입고된 수량(INSERT된 자료 AMOUNT)
    WHERE PCODE = 입고된상품번호(INSERT된 자료의 PCODE);
    
    -상품이 출도된 경우 -> 해당 상품을 찾아서 재고수량 감소UPDATE
    UPDATE TB_PRODUCT
...
--210번번상품이 오늘날짜로 7개 출고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL, 210, SYSDATE, 7 ,'출고');

--200번번상품이 오늘날짜로 100개 출고
INSERT INTO TB_PRODETAIL
